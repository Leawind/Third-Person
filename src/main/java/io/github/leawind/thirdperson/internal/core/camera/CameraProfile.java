...
public record CameraProfile(
  ...
  public CameraParameters cameraParameters(CameraSubjectDimensions subjectDimensions) {
    double resolvedDistance =
        subjectDimensions.resolveDistance(distanceFactor, fovMultiplier);
    return centered
        ? new CameraParameters(resolvedDistance, 0.0, centeredOffsetY)
        : new CameraParameters(resolvedDistance, offsetX, offsetY);
  }
