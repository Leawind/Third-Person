...
public final class PlayerRotationController {
  ...
  private PlayerRotationDecision decision;
  ...
  public LookRotation update(
    ...
    if (decision.smoothing() == PlayerRotationSmoothing.FRAME_EXPONENTIAL) {
  ...
  public LookRotation updateFrame(LookRotation target, double deltaSeconds) {
    ...
    if (!initialized
        || decision == null
        || decision.smoothing() != PlayerRotationSmoothing.FRAME_EXPONENTIAL) {
      throw new IllegalStateException("Frame-exponential rotation has not been initialized");
    }
    double alpha = ExponentialSmoothing.alpha(deltaSeconds, decision.halfLifeSeconds());
    yawDegrees =
        wrapDegrees(
            yawDegrees + (float) (shortestDegrees(target.yawDegrees() - yawDegrees) * alpha));
    pitchDegrees =
  ...
  public Optional<PlayerRotationDecision> decision() {
