...
public class Config {
	...
	private static final ForgeConfigSpec.DoubleValue CAMERA_OFFSET_RATIO_X   = BUILDER.comment("玩家在屏幕中的水平偏移，0表示屏幕中间，1表示边缘（左或右）")
																					  .defineInRange("camera_offset_ratio_x",
																									 0.5,
																									 0,
																									 1);
	private static final ForgeConfigSpec.DoubleValue CAMERA_OFFSET_RATIO_Y   = BUILDER.comment("玩家在屏幕中的垂直偏移，0表示屏幕中间，1表示屏幕底部边缘")
																					  .defineInRange("camera_offset_ratio_y",
																									 +0.372,
																									 0,
																									 1);
	...
	private static final ForgeConfigSpec.DoubleValue AIM_OFFSET     = BUILDER.comment("瞄准时相机视线与玩家视线的距离")
																			 .defineInRange("aim_camera_offset_distance",
																							0.7,
																							0.0,
																							4.0);
	private static final ForgeConfigSpec.DoubleValue AIM_OFFSET_TOP = BUILDER.comment("使用头顶机位瞄准时，相机视线与玩家视线的距离")
																			 .defineInRange("aim_camera_offset_distance_top",
																							0.6,
																							0.0,
																							4.0);
	...
	private static final ForgeConfigSpec.DoubleValue MOVE_TO_SURFACE_SPEED       = BUILDER.comment("相机远离玩家的速度系数")
																						  .defineInRange("move_to_surface_speed",
																										 0.5,
																										 0D,
																										 1.0);
	...
	public static void onLoad(final ModConfigEvent event){
		...
		smooth_eye_speed = new Vec3(SMOOTH_EYE_SPEED_XZ.get(), SMOOTH_EYE_SPEED_Y.get(), SMOOTH_EYE_SPEED_XZ.get());
