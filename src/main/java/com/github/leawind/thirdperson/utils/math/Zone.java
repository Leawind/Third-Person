...
public class Zone {
   ...
   * @param scale 比例
   */
  public @NonNull Zone scale(double scale) {
   ...
   */
  public @NonNull Zone intersection(@NonNull Zone zone) throws IllegalArgumentException {
   ...
   * @throws IllegalArgumentException 没有相交区域
   */
  public @NonNull Zone union(@NonNull Zone zone) {
   ...
   */
  public @NonNull Zone expendRadius(double d) throws IllegalArgumentException {
    if (d < 0) {
   ...
   */
  public @NonNull Zone expend(double a, double b) {
    if (a < 0 || b < 0) {
   ...
   */
  public @NonNull Zone squeeze(double d) throws IllegalArgumentException {
    if (d > radius()) {
   ...
   */
  public @NonNull Zone squeezeSafely(double d) {
    double r = Math.min(d, radius());
    return Zone.ofAuto(min + r, max - r);
  }

  /** 检查当前区间是否与给定的区间有交集。 */
  public boolean hasIntersection(@NonNull Zone zone) {
   ...
   * @param offset 偏移量
   */
  public @NonNull Zone move(double offset) {
    return new Zone(min + offset, max + offset);
  }

  /** 检查当前区间是否完全位于给定区间的左侧。 */
  public boolean lessThan(@NonNull Zone zone) {
    return max <= zone.min;
  }

  /** 检查当前区间是否完全位于给定区间的右侧。 */
  public boolean greaterThan(@NonNull Zone zone) {
    return min >= zone.max;
  }
   ...
   */
  public @NonNull Zone lessNeighbor(double length) throws IllegalArgumentException {
    if (length < 0) {
   ...
   * @throws IllegalArgumentException length < 0
   */
  public @NonNull Zone greaterNeighbor(double length) throws IllegalArgumentException {
    if (length < 0) {
      throw new IllegalArgumentException("Length must be non-negative, not " + length);
    }
  ...
  /** 检查当前区间是否完全包含于给定区间中。 */
  public boolean in(@NonNull Zone zone) {
    return zone.min <= min && max <= zone.max;
  }

  /** 检查当前区间是否完全包含给定区间。 */
  public boolean contains(@NonNull Zone zone) {
   ...
   * @throws IllegalArgumentException min < max
   */
  public @NonNull Zone withMax(double max) throws IllegalArgumentException {
   ...
   * @throws IllegalArgumentException min < max
   */
  public @NonNull Zone withMin(double min) throws IllegalArgumentException {
    return new Zone(min, max);
  }
   ...
   * @throws IllegalArgumentException min > max
   */
  public static @NonNull Zone of(double min, double max) throws IllegalArgumentException {
    return new Zone(min, max);
  }
   ...
   * @param b 另一个端点
   */
  public static @NonNull Zone ofAuto(double a, double b) {
   ...
   */
  public static @NonNull Zone ofRadius(double center, double radius)
   ...
   * @throws IllegalArgumentException length < 0
   */
  public static @NonNull Zone ofLength(double center, double length)
      throws IllegalArgumentException {
