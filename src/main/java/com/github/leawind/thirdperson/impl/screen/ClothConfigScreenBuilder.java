...
import com.github.leawind.thirdperson.impl.config.AbstractConfig;
...
public class ClothConfigScreenBuilder extends ConfigScreenBuilder {
  public @NotNull Screen build(@NotNull Config config, @Nullable Screen parent) {
    ...
    // ===============================================================================================================//
    final var CATEGORY_COMMON =
        builder.getOrCreateCategory(ConfigManager.getText("option_category.common"));
    {
      CATEGORY_COMMON.addEntry(
          buildBooleanEntry(
              "is_mod_enabled",
      ...
      SUBCATEGORY_PLAYER_ROTATION.add(
          entryBuilder
              .startEnumSelector(
                  ConfigManager.getText(AbstractConfig.PlayerRotateMode.KEY),
                  AbstractConfig.PlayerRotateMode.class,
                  config.normal_rotate_mode)
              .setEnumNameProvider(AbstractConfig.PlayerRotateMode::formatter)
              .setSaveConsumer(v -> config.normal_rotate_mode = v)
              .build());
      SUBCATEGORY_PLAYER_ROTATION.add(
          buildBooleanEntry(
      ...
      Subcategory_Player_Fade_Out.add(
          buildDoubleEntry(
              ...
              0D,
              1D,
              defaults.player_invisible_threshold,
      ...
      Subcategory_Camera_Distance_Adjustment.add(
          buildIntSliderEntry(
              ...
              2,
              64,
              defaults.available_distance_count,
      ...
      Subcategory_Camera_Distance_Adjustment.add(
          buildDoubleEntry(
              ...
              0,
              6D,
              defaults.camera_distance_min,
              config.camera_distance_min,
              v -> config.camera_distance_min = Math.min(v, config.camera_distance_max),
              entryBuilder));
      Subcategory_Camera_Distance_Adjustment.add(
          buildDoubleEntry(
              "camera_distance_max",
        ...
        builder.getOrCreateCategory(ConfigManager.getText("option_category.smoothness"));
    {
      ...
      Subcategory_Adjusting_Camera.add(
          buildSmoothHalflifeEntry(
    ...
    final var CATEGORY_CAMERA_OFFSET =
        builder.getOrCreateCategory(ConfigManager.getText("option_category.camera_offset"));
    {
      CATEGORY_CAMERA_OFFSET.addEntry(
          buildDoubleEntry(
              "aiming_fov_divisor",
              1D,
              2D,
              defaults.aiming_fov_divisor,
    ...
    final var CATEGORY_AIMING_CHECK =
        builder.getOrCreateCategory(ConfigManager.getText("option_category.item_predicates"));
    {
      CATEGORY_AIMING_CHECK.addEntry(
          buildBooleanEntry(
    ...
    // ===============================================================================================================//
    final var CATEGORY_OTHER =
        builder.getOrCreateCategory(ConfigManager.getText("option_category.other"));
    {
      if (getAvailableBuidlers().size() > 1) {
        CATEGORY_OTHER.addEntry(
            entryBuilder
                .startDropdownMenu(
                    ConfigManager.getText("option.config_screen_api"),
                    config.config_screen_api,
                    v -> config.config_screen_api = v)
                .setSelections(getAvailableBuidlers().keySet())
                .build());
      }
      CATEGORY_OTHER.addEntry(
          entryBuilder
              .startEnumSelector(
                  ConfigManager.getText(AbstractConfig.CameraDistanceMode.KEY),
                  AbstractConfig.CameraDistanceMode.class,
                  config.camera_distance_mode)
              .setEnumNameProvider(AbstractConfig.CameraDistanceMode::formatter)
              .setSaveConsumer(v -> config.camera_distance_mode = v)
              .build());
      CATEGORY_OTHER.addEntry(
          buildDoubleEntry(
              ...
              -0.5,
              0.5,
              defaults.rotate_center_height_offset,
      ...
      CATEGORY_OTHER.addEntry(
          buildDoubleEntry(
              ...
              32D,
              2048D,
              defaults.camera_ray_trace_length,
  ...
  private BooleanToggleBuilder booleanEntry(
      String name,
    ...
    return entryBuilder
        .startBooleanToggle(ConfigManager.getText("option." + name), currentValue)
        .setTooltip(ConfigManager.getText("option." + name + ".desc"))
        .setDefaultValue(defaultValue)
        .setSaveConsumer(setter);
  }

  private BooleanListEntry buildBooleanEntry(
      String name,
      boolean defaultValue,
      boolean currentValue,
      Consumer<Boolean> setter,
  ...
  private SubCategoryBuilder buildSubCategory(String name, ConfigEntryBuilder entryBuilder) {
    return entryBuilder
        .startSubCategory(ConfigManager.getText("option_group." + name))
        .setExpanded(true)
        .setTooltip(ConfigManager.getText("option_group." + name + ".desc"));
  }

  private IntegerSliderEntry buildIntSliderEntry(
      String name,
      int min,
      int max,
      int defaultValue,
      ...
      ConfigEntryBuilder entryBuilder) {
    return entryBuilder
        .startIntSlider(ConfigManager.getText("option." + name), currentValue, min, max)
        .setTooltip(ConfigManager.getText("option." + name + ".desc"))
        .setDefaultValue(defaultValue)
        .setSaveConsumer(setter)
        .build();
  ...
  private DoubleListEntry buildDoubleEntry(
      String name,
      double min,
      double max,
      double defaultValue,
      ...
      ConfigEntryBuilder entryBuilder) {
    return entryBuilder
        .startDoubleField(ConfigManager.getText("option." + name), currentValue)
        .setTooltip(ConfigManager.getText("option." + name + ".desc"))
        .setDefaultValue(defaultValue)
        .setSaveConsumer(setter)
        .setMin(min)
  ...
  private DoubleListEntry buildSmoothHalflifeEntry(
      String name,
      double defaultValue,
      double currentValue,
      Consumer<Double> setter,
  ...
  private StringListListEntry buildStringListEntry(
      ...
      ConfigEntryBuilder entryBuilder) {
    return entryBuilder
        .startStrList(ConfigManager.getText("option." + name), currentValue)
        .setTooltip(ConfigManager.getText("option." + name + ".desc"))
        .setSaveConsumer(setter)
        .setDefaultValue(defaultValue)
        .setDeleteButtonEnabled(true)
        .setCellErrorSupplier(ItemPredicateUtil::supplyError)
        .setExpanded(true)
        .build();
  }
}
