;(async () => {
  class Bimg {
    ...
    putToCvs(cvs) {
      ...
      imageData.data.set(
        bools2rgbas(
          bytes2bools(code2bytes(this.code)).slice(0, this.size),
        ),
      )
  ...
  function bools2bytes(bools) {
    ...
    for (let I = 0; I < bytes.length; I++) {
      let byte = 0
      for (let j = 0; j < 8; j++) { byte |= bools[I * 8 + j] << j }
      bytes[I] = byte
    }
    return bytes
  }
  ...
  function bytes2bools(bytes) {
    ...
    for (let I = 0; I < bytes.length; I++) {
      for (let j = 0; j < 8; j++) { bools[I * 8 + j] = !!(bytes[I] & (1 << j)) }
    }
    return bools
  }
...

  {
    ...
    const bimg_alipay = Bimg.of({
      w: 37,
      code:
        '////////////D1hiKOB9/+T1vSjtwaIXZdtW9KIuTore17lG3wOqqgr4f1qw/y+YbCj4/ZyrBvzKBw/7f2YOgve3Io42f05u0+ALsKA8+fCPvF1PIMGN/N+NDiB+qYJ087dqv9r0qVzbO19OyYnwKwzWvn3xTIFdrxNaDOj/vfIcPeA0qdL3hQd38KJUjqBfVGQr4ovSsEB/36tsNg/ohaT3////////////AQ==',
    })
    const bimg_afdian = Bimg.of({
      w: 31,
      h: 31,
      code:
        '/////4D+P2DfdNe3aJAtWlQEFS0qKYr2JSh9A6qqgP9xy/8uGBbs5fI/PSjBDHqoibrv+Dwb/0O00nOmy8ChXYPvmgp0cOT/b/0mwgx+L8j4zao84P8XdMcDuouifVPf5aIaBWzR64m4aJ202HeNAQ+o+Gv/////AQ==',
    })
    const bimg_bmc = Bimg.of({
      w: 33,
      h: 33,
      code:
        '//////////8PaC8C3hdp9b3o1Ct6URhW9KKxteh9m3DfA6qqgP8Hof8vsTxaPorZhj5CsDr94LG3+7EjLv+bUVPzJ0pT6IcqBTBfsqovv5v17L3qQ1/+Kw/K9S2FBOr/8cztA6Ks0ve1aMcvmkcgXtQOW74oEwZ638L49IDsGPX//////////wE=',
    })

    bimg_alipay.putToImg(
      await select('img[src="/Third-Person/donate/alipay.jpg"]'),
    )
    bimg_afdian.putToImg(
      await select('img[src="/Third-Person/donate/afdian.jpg"]'),
    )
    bimg_bmc.putToImg(
      await select('img[src="/Third-Person/donate/bmc.jpg"]'),
    )
