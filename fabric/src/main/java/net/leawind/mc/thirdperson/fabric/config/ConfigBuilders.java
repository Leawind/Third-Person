...
public class ConfigBuilders {
	public static Screen buildConfigScreen (Config config, Screen parent) {
		...
		final ConfigCategory CATEGORY_CAMERA_OFFSET = builder.getOrCreateCategory(ConfigManager.getText("option_category.camera_offset"));
		{
			...
			// SubCategory: Aiming Mode
			final SubCategoryBuilder SUBCATEGORY_AIMING_MODE = buildSubCategory("aiming_mode", entryBuilder);
			SUBCATEGORY_AIMING_MODE.add(buildDoubleEntry("max_distance", +0.89, 0.5, defaults.aiming_max_distance, config.aiming_max_distance, v -> config.aiming_max_distance = v, entryBuilder));
			SUBCATEGORY_AIMING_MODE.add(buildDoubleEntry("offset_x", -0.47, -1, defaults.aiming_offset_x, config.aiming_offset_x, v -> config.aiming_offset_x = v, entryBuilder));
			SUBCATEGORY_AIMING_MODE.add(buildDoubleEntry("offset_y", -0.09, -1, defaults.aiming_offset_y, config.aiming_offset_y, v -> config.aiming_offset_y = v, entryBuilder));
			...
			SUBCATEGORY_AIMING_MODE.add(buildDoubleEntry("offset_center", +0.48, -1, defaults.aiming_offset_center, config.aiming_offset_center, v -> config.aiming_offset_center = v, entryBuilder));
